...
13b. Extract `{job_id}` from the source URL if available (e.g. LinkedIn `/jobs/view/{job_id}`, Greenhouse `/jobs/{job_id}`, Ashby UUID, Lever UUID). If no ID extractable, omit the `-{job_id}` suffix.
13c. Normalize `{role}` to kebab-case lowercase (e.g. "Software Engineer III - React" → "software-engineer-iii-react"), truncated to 50 chars max.
14. Escribe HTML a `/tmp/cv-{candidate}-{company}-{role}.html`
15. Ejecuta: `node generate-pdf.mjs /tmp/cv-{candidate}-{company}-{role}.html output/{candidate}-{company}-{role}-{job_id}.pdf --format={letter|a4}`
    If no `{job_id}`: `output/{candidate}-{company}-{role}.pdf`
...
| `{{TARGET_ROLE}}` | The target role title from the JD (e.g. "Software Engineer" or "Frontend Engineer"). Appears under the name in smaller font. |
...
b. **IMMEDIATELY** download the PDF using Bash:
   ```bash
   curl -sL -o "output/{candidate}-{company}-{role}-{job_id}-canva.pdf" "{download_url}"
   ```
   The export URL is a pre-signed S3 link that expires in ~2 hours. Download it right away.
c. Verify the download:
   ```bash
   file output/{candidate}-{company}-{role}-{job_id}-canva.pdf
   ```
